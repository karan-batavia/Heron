...
export async function startServer(config: ServerConfig): Promise<void> {
  ...
  server.listen(config.port, config.host, () => {
    ...
    logger.raw('');
    logger.raw('  ┌──────────────────────────────────────────────────────');
    logger.raw('  │ I need you to complete a security access review for');
    logger.raw('  │ this project. Make sequential API calls to the Heron');
    logger.raw('  │ endpoint until it says "Interview complete".');
    logger.raw('  │');
    ...
    logger.raw('  │   "content":"Hi, I am ready to answer questions"}]}');
    logger.raw('  │');
    logger.raw('  │ Step 2 — Send each answer:');
    logger.raw(`  │ POST ${baseUrl}/v1/chat/completions`);
    logger.raw('  │ Body: {"model":"any","messages":[{"role":"user",');
    logger.raw('  │   "content":"YOUR ANSWER"}],');
    logger.raw('  │   "heron_session_id":"SESSION_ID_FROM_STEP_1"}');
    logger.raw('  │');
    logger.raw('  │ Step 3 — Repeat until "Interview complete".');
    logger.raw('  │ Answer about THIS project. Never reveal secrets.');
    logger.raw('  └──────────────────────────────────────────────────────');
    logger.raw('');
