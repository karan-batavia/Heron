...
export async function startServer(config: ServerConfig): Promise<void> {
  ...
  server.listen(config.port, config.host, () => {
    ...
    logger.log('');
    logger.log('  ┌──────────────────────────────────────────────────────');
    logger.log('  │ I need you to complete a security access review for this project.');
    logger.log('  │ Make sequential API calls to the Heron endpoint until it says');
    logger.log('  │ "Interview complete".');
    logger.log('  │');
    logger.log(`  │ Step 1 — Start the interview:`);
    logger.log(`  │ POST ${baseUrl}/v1/chat/completions`);
    logger.log('  │ Body: {"model":"any","messages":[{"role":"user","content":');
    logger.log('  │   "Hi, I am ready to answer questions about this project."}]}');
    logger.log('  │');
    logger.log('  │ Step 2 — Send each answer:');
    logger.log(`  │ POST ${baseUrl}/v1/chat/completions`);
    logger.log('  │ Body: {"model":"any","messages":[{"role":"user","content":');
    logger.log('  │   "YOUR ANSWER"}],"heron_session_id":"SESSION_ID_FROM_STEP_1"}');
    logger.log('  │');
    logger.log('  │ Step 3 — Repeat step 2 until "Interview complete".');
    logger.log('  │');
    logger.log('  │ Answer about THIS project. Never reveal actual secret values.');
    logger.log('  └──────────────────────────────────────────────────────');
    logger.log('');
